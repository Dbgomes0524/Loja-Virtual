...
const criarCards = (listaProdutos) => {
    ...
    listaProdutos.forEach((elem) => {
        ...
        btnAdd.addEventListener('click', () => {
            ...
            const itemExiste = cart.find(el => el.id === elem.idProduto)
            if(itemExiste){
                itemExiste.qtd++
            } else{
                cart.push({
                ...
                id: elem.idProduto,
                img: elem.caminhoImagem,
                desc: elem.descricao,
                qtd: 1,
                valor: elem.valorUnitario,
                uni: elem.unidade
            })
            }

            // window.location = "carrinho.html" pra eu levar ele pra pagina do carrinho
            saveLocalStorage()
            ;
        });
